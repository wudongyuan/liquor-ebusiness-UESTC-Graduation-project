...
function addPro(){
	...
	if(is_array($uploadFiles)&&$uploadFiles){
		foreach($uploadFiles as $key=>$uploadFile){
			...
			thumb($path."/".$uploadFile['name'],"../image_400/".$uploadFile['name'],400,533);
	...
	if($res&&$pid){
		...
		foreach($uploadFiles as $uploadFile){
			...
			if(file_exists("../image_400/".$uploadFile['name'])){
				unlink("../image_400/".$uploadFile['name']);
			}
		}
...
function getProsByCid($cid){
	$sql="select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName,p.cId from pro as p join cate c on p.cId=c.id where p.cId={$cid} limit 5";
	$rows=fetchAll($sql);
	return $rows;
}
...
function getSmallProsByCid($cid){
	$sql="select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName,p.cId from pro as p join cate c on p.cId=c.id where p.cId={$cid} limit 4,4";
	$rows=fetchAll($sql);
	return $rows;
}
