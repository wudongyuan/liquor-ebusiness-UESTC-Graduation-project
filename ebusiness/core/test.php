...
function getProsByCid($cid){
		$sql="select p.id,p.pName,p.iPrice from pro as p join cate c on p.cId=c.id where p.cid={$cid} limit 4";
		$rows=fetchAll($sql);
		return $rows;
}
...
function getSmallProsByCid($cid){
		$sql="select p.id,p.pName,p.iPrice from pro as p join cate c on p.cId=c.id where p.cid={$cid} limit 4,4";
		$rows=fetchAll($sql);
		return $rows;
}
